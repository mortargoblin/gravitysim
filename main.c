...
void spawn_planet(Body *bodies, int *body_count) {
  float vel_x = ((float)rand() / RAND_MAX) * 200 - 100;
  float vel_y = ((float)rand() / RAND_MAX) * 200 - 100;
  ...
  while (!WindowShouldClose()) {
    ...
    if (IsMouseButtonDown(MOUSE_BUTTON_RIGHT)) {
      for (int i = 0; i < body_count; i++) { // :DD
        bodies[i].pos = Vector2Add(bodies[i].pos, GetMouseDelta());
      }
    }

    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      spawn_planet(bodies, &body_count);
    }
    ...
    BeginDrawing();
    {
      ...
      DrawText(
          TextFormat(
            ...
            "FRAME TIME\n%f\n\nBODIES\n%d\n\n"
            "EARTH VELOCITY\nX: %.2f\nY: %.2f",
            body_count, dt, bodies[1].vel.x, bodies[1].vel.y
            ),
          10, 10, 20, WHITE
          );
    }
